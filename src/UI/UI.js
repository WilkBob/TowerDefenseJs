...
export class UI {
  constructor() {
    ...
    this.healthBar = new healthBar(
      ...
      200,
      20,
      global.playerDefinition.health,
    );
  ...
  update() {
    ...
    if (moneyDifference !== 0) {
      ...
      let scalingFactor = Math.ceil(
        Math.max(1, Math.abs(moneyDifference) / 10),
      );
      this.state.money +=
        Math.sign(moneyDifference) *
        Math.min(Math.abs(moneyDifference), scalingFactor);
    }
  ...
  drawMouse() {
    ...
    if (global.game.paused) {
      ...
      global.mouse.canPlace = global.game.clickmask.check(
        global.mouse.x,
        global.mouse.y,
      );

      // Find the first tower the mouse is over
      const towerOver = global.game.objects.towers.find(
        (tower) => tower.isMouseOver,
      );
      ...
      global.ctx.fillStyle = !global.mouse.canPlace
        ? "rgba(255,0,0,0.5)"
    ...
    global.ctx.arc(
      global.mouse.x * global.size,
      global.mouse.y * global.size,
      global.mouse.radius * global.fac,
      0,
      Math.PI * 2,
    );
    global.ctx.fill();
  ...
  draw() {
    ...
    if (this.view.hud) {
      global.ctx.fillText(
        `Wave: ${global.game.levelcontroller.waveNumber - 1 || 1}`,
        10 * global.fac,
        80 * global.fac,
      );
      global.ctx.fillText(
