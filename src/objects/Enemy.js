...
export class Enemy {
  constructor(path, definition, last = false) {
    this.path = path;
    this.pathIndex = 0;
    ...
    this.currentMessage =
      this.messages.spawn[
        Math.floor(Math.random() * this.messages.spawn.length)
      ];
    ...
    global.game.ui.messages.addMessage(
      this.currentMessage,
  ...
  drawHealthBar() {
    ...
    this.ctx.fillRect(
      healthBarX,
      healthBarY,
      healthBarWidth * (this.health / this.definition.health),
      healthBarHeight,
    );
  }
  ...
  update() {
    if (this.dead || this.health <= 0) {
      ...
      this.clearIntervals();
      global.game.objects.enemies = global.game.objects.enemies.filter(
        (enemy) => enemy !== this,
      );
      global.game.objects.spritesheets.push(
        new SpriteSheet(
          this.deathAnimation.src,
          this.deathAnimation.size,
          this.deathAnimation.frames,
          { x: this.x, y: this.y, rotation: this.rotation },
          this.deathAnimation.fac,
        ),
      );
      if (global.game.objects.enemies.length === 0) {
        global.game.levelcontroller.waveComplete();
      }
      return;
    }
    ...
    if (distance < adjustedSpeed / global.size) {
      ...
      if (this.pathIndex >= this.path.length) {
        this.currentMessage =
          this.messages.hitTower[
            Math.floor(Math.random() * this.messages.hitTower.length)
          ];
